...
1. Load the `thermo-nuclear-review` skill (via the `read` tool on its SKILL.md) and follow it exactly: scope (only added/modified code), breaking functionality and devex, feature leaks, intended breakage, over-reporting, final response / PR discussion rules, critical rules.
